...
import {
  BulkCreateInspectionDto,
  BulkUpdateStatusInspectionDto,
  CreateInspectionDto,
  PaginatedInspectionResponseDto,
  QueryInspectionDto,
  UpdateInspectionDto,
...
export class InspectionService {
  ...
  constructor(
    ...
    private readonly repository: Repository<InspectionReport>,
  ) {}

  async create(dto: CreateInspectionDto, userId: string): Promise<InspectionReport> {
    this.logger.log(`Creating inspections by ${userId}`);
    ...
    this.logger.debug(`Created inspections ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateInspectionDto, userId: string): Promise<InspectionReport[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryInspectionDto): Promise<PaginatedInspectionResponseDto> {
    const page = query.page || 1;
    const limit = Math.min(query.limit || 20, 200);
    const where: FindOptionsWhere<InspectionReport> = {};
    ...
    const qb = this.repository.createQueryBuilder('inspections');
    qb.where(where);

    if (query.search) {
      qb.andWhere(
        '(inspections.notes ILIKE :search OR inspections.externalRef ILIKE :search OR inspections.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('inspections.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`inspections.${sortBy}`, sortOrder);
  ...
  async update(id: string, dto: UpdateInspectionDto, userId: string): Promise<InspectionReport> {
  ...
  async restore(id: string, userId: string): Promise<InspectionReport> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusInspectionDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    const qb = this.repository
      ...
      .select('inspections.status', 'status')
      .addSelect('COUNT(*)', 'count')
      .where('inspections.isDeleted = false')
      .groupBy('inspections.status');
    if (organizationId) {
      qb.andWhere('inspections.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<InspectionReport[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<InspectionReport[]> {
    const qb = this.repository.createQueryBuilder('inspections');
    qb.where('inspections.isDeleted = false');
    if (organizationId) {
      qb.andWhere('inspections.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`inspections.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QueryInspectionDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<InspectionReport> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<InspectionReport> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<InspectionReport> {
    const entity = await this.findOne(id);
