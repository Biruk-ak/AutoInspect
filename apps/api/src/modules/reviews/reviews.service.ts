...
export class ReviewService {
  ...
  async create(dto: CreateReviewDto, userId: string): Promise<ServiceReview> {
    this.logger.log(`Creating reviews by ${userId}`);
    ...
    this.logger.debug(`Created reviews ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateReviewDto, userId: string): Promise<ServiceReview[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryReviewDto): Promise<PaginatedReviewResponseDto> {
    const page = query.page || 1;
    const limit = Math.min(query.limit || 20, 200);
    const where: FindOptionsWhere<ServiceReview> = {};
    ...
    const qb = this.repository.createQueryBuilder('reviews');
    qb.where(where);

    if (query.search) {
      qb.andWhere(
        '(reviews.notes ILIKE :search OR reviews.externalRef ILIKE :search OR reviews.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('reviews.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`reviews.${sortBy}`, sortOrder);
  ...
  async update(id: string, dto: UpdateReviewDto, userId: string): Promise<ServiceReview> {
  ...
  async restore(id: string, userId: string): Promise<ServiceReview> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusReviewDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    ...
    if (organizationId) {
      qb.andWhere('reviews.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<ServiceReview[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<ServiceReview[]> {
    const qb = this.repository.createQueryBuilder('reviews');
    ...
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`reviews.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QueryReviewDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<ServiceReview> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<ServiceReview> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<ServiceReview> {
    const entity = await this.findOne(id);
