...
export class AuditService {
  ...
  async create(dto: CreateAuditDto, userId: string): Promise<AuditLog> {
    this.logger.log(`Creating audit by ${userId}`);
    ...
    this.logger.debug(`Created audit ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateAuditDto, userId: string): Promise<AuditLog[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryAuditDto): Promise<PaginatedAuditResponseDto> {
    ...
    if (query.search) {
      qb.andWhere(
        '(audit.notes ILIKE :search OR audit.externalRef ILIKE :search OR audit.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('audit.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
  ...
  async update(id: string, dto: UpdateAuditDto, userId: string): Promise<AuditLog> {
  ...
  async restore(id: string, userId: string): Promise<AuditLog> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusAuditDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<AuditLog[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<AuditLog[]> {
    const qb = this.repository.createQueryBuilder('audit');
    qb.where('audit.isDeleted = false');
    if (organizationId) {
      qb.andWhere('audit.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`audit.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }
  ...
  async clone(id: string, userId: string): Promise<AuditLog> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<AuditLog> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<AuditLog> {
    const entity = await this.findOne(id);
