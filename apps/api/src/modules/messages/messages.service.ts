...
import {
  BulkCreateMessageDto,
  BulkUpdateStatusMessageDto,
  CreateMessageDto,
...
export class MessageService {
  ...
  async create(dto: CreateMessageDto, userId: string): Promise<ChatMessage> {
    this.logger.log(`Creating messages by ${userId}`);
    ...
    this.logger.debug(`Created messages ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateMessageDto, userId: string): Promise<ChatMessage[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryMessageDto): Promise<PaginatedMessageResponseDto> {
    ...
    const qb = this.repository.createQueryBuilder('messages');
    qb.where(where);

    if (query.search) {
      qb.andWhere(
        '(messages.notes ILIKE :search OR messages.externalRef ILIKE :search OR messages.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('messages.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`messages.${sortBy}`, sortOrder);
  ...
  async update(id: string, dto: UpdateMessageDto, userId: string): Promise<ChatMessage> {
  ...
  async restore(id: string, userId: string): Promise<ChatMessage> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusMessageDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    const qb = this.repository
      ...
      .select('messages.status', 'status')
      .addSelect('COUNT(*)', 'count')
      .where('messages.isDeleted = false')
      .groupBy('messages.status');
    if (organizationId) {
      qb.andWhere('messages.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<ChatMessage[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<ChatMessage[]> {
    const qb = this.repository.createQueryBuilder('messages');
    qb.where('messages.isDeleted = false');
    if (organizationId) {
      qb.andWhere('messages.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`messages.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QueryMessageDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<ChatMessage> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<ChatMessage> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<ChatMessage> {
    const entity = await this.findOne(id);
