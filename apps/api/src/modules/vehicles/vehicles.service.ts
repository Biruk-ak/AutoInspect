...
export class VehicleService {
  ...
  async create(dto: CreateVehicleDto, userId: string): Promise<Vehicle> {
    this.logger.log(`Creating vehicles by ${userId}`);
    ...
    this.logger.debug(`Created vehicles ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateVehicleDto, userId: string): Promise<Vehicle[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryVehicleDto): Promise<PaginatedVehicleResponseDto> {
    ...
    const qb = this.repository.createQueryBuilder('vehicles');
    qb.where(where);

    if (query.search) {
      qb.andWhere(
        '(vehicles.notes ILIKE :search OR vehicles.externalRef ILIKE :search OR vehicles.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('vehicles.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`vehicles.${sortBy}`, sortOrder);
  ...
  async restore(id: string, userId: string): Promise<Vehicle> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusVehicleDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    const qb = this.repository
      ...
      .select('vehicles.status', 'status')
      .addSelect('COUNT(*)', 'count')
      .where('vehicles.isDeleted = false')
      .groupBy('vehicles.status');
    if (organizationId) {
      qb.andWhere('vehicles.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<Vehicle[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<Vehicle[]> {
    const qb = this.repository.createQueryBuilder('vehicles');
    qb.where('vehicles.isDeleted = false');
    if (organizationId) {
      qb.andWhere('vehicles.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`vehicles.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QueryVehicleDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<Vehicle> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<Vehicle> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<Vehicle> {
    const entity = await this.findOne(id);
