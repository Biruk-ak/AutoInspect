...
export class UserService {
  ...
  async create(dto: CreateUserDto, userId: string): Promise<UserAccount> {
    this.logger.log(`Creating users by ${userId}`);
    ...
    this.logger.debug(`Created users ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateUserDto, userId: string): Promise<UserAccount[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryUserDto): Promise<PaginatedUserResponseDto> {
    ...
    if (query.search) {
      qb.andWhere(
        '(users.notes ILIKE :search OR users.externalRef ILIKE :search OR users.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }
  ...
  async update(id: string, dto: UpdateUserDto, userId: string): Promise<UserAccount> {
  ...
  async restore(id: string, userId: string): Promise<UserAccount> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusUserDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    ...
    if (organizationId) {
      qb.andWhere('users.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<UserAccount[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<UserAccount[]> {
    const qb = this.repository.createQueryBuilder('users');
    qb.where('users.isDeleted = false');
    if (organizationId) {
      qb.andWhere('users.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`users.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QueryUserDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<UserAccount> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<UserAccount> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<UserAccount> {
    const entity = await this.findOne(id);
