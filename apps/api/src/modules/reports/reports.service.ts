...
export class ReportService {
  ...
  constructor(
    ...
    private readonly repository: Repository<GeneratedReport>,
  ) {}

  async create(dto: CreateReportDto, userId: string): Promise<GeneratedReport> {
    this.logger.log(`Creating reports by ${userId}`);
    ...
    this.logger.debug(`Created reports ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateReportDto, userId: string): Promise<GeneratedReport[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryReportDto): Promise<PaginatedReportResponseDto> {
    const page = query.page || 1;
    const limit = Math.min(query.limit || 20, 200);
    const where: FindOptionsWhere<GeneratedReport> = {};
    ...
    const qb = this.repository.createQueryBuilder('reports');
    qb.where(where);

    if (query.search) {
      qb.andWhere(
        '(reports.notes ILIKE :search OR reports.externalRef ILIKE :search OR reports.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('reports.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`reports.${sortBy}`, sortOrder);
  ...
  async findOne(id: string): Promise<GeneratedReport> {
  ...
  async update(id: string, dto: UpdateReportDto, userId: string): Promise<GeneratedReport> {
  ...
  async restore(id: string, userId: string): Promise<GeneratedReport> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusReportDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    const qb = this.repository
      ...
      .select('reports.status', 'status')
      .addSelect('COUNT(*)', 'count')
      .where('reports.isDeleted = false')
      .groupBy('reports.status');
    if (organizationId) {
      qb.andWhere('reports.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<GeneratedReport[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<GeneratedReport[]> {
    const qb = this.repository.createQueryBuilder('reports');
    qb.where('reports.isDeleted = false');
    if (organizationId) {
      qb.andWhere('reports.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`reports.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }
  ...
  async clone(id: string, userId: string): Promise<GeneratedReport> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<GeneratedReport> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<GeneratedReport> {
    const entity = await this.findOne(id);
