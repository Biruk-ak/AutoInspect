...
export class InventoryService {
  ...
  constructor(
    @InjectRepository(PartsInventory)
    private readonly repository: Repository<PartsInventory>,
  ) {}

  async create(dto: CreateInventoryDto, userId: string): Promise<PartsInventory> {
    this.logger.log(`Creating inventory by ${userId}`);
    ...
    this.logger.debug(`Created inventory ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateInventoryDto, userId: string): Promise<PartsInventory[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryInventoryDto): Promise<PaginatedInventoryResponseDto> {
    const page = query.page || 1;
    const limit = Math.min(query.limit || 20, 200);
    const where: FindOptionsWhere<PartsInventory> = {};
    ...
    if (query.search) {
      qb.andWhere(
        '(inventory.notes ILIKE :search OR inventory.externalRef ILIKE :search OR inventory.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('inventory.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`inventory.${sortBy}`, sortOrder);
  ...
  async findOne(id: string): Promise<PartsInventory> {
  ...
  async update(id: string, dto: UpdateInventoryDto, userId: string): Promise<PartsInventory> {
  ...
  async restore(id: string, userId: string): Promise<PartsInventory> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusInventoryDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    ...
    if (organizationId) {
      qb.andWhere('inventory.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<PartsInventory[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<PartsInventory[]> {
    const qb = this.repository.createQueryBuilder('inventory');
    qb.where('inventory.isDeleted = false');
    if (organizationId) {
      qb.andWhere('inventory.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`inventory.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QueryInventoryDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<PartsInventory> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<PartsInventory> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<PartsInventory> {
    const entity = await this.findOne(id);
