...
export class CustomerService {
  ...
  async create(dto: CreateCustomerDto, userId: string): Promise<CustomerProfile> {
    this.logger.log(`Creating customers by ${userId}`);
    ...
    this.logger.debug(`Created customers ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateCustomerDto, userId: string): Promise<CustomerProfile[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryCustomerDto): Promise<PaginatedCustomerResponseDto> {
    const page = query.page || 1;
    const limit = Math.min(query.limit || 20, 200);
    const where: FindOptionsWhere<CustomerProfile> = {};
    ...
    const qb = this.repository.createQueryBuilder('customers');
    qb.where(where);

    if (query.search) {
      qb.andWhere(
        '(customers.notes ILIKE :search OR customers.externalRef ILIKE :search OR customers.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('customers.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`customers.${sortBy}`, sortOrder);
  ...
  async findOne(id: string): Promise<CustomerProfile> {
  ...
  async update(id: string, dto: UpdateCustomerDto, userId: string): Promise<CustomerProfile> {
  ...
  async restore(id: string, userId: string): Promise<CustomerProfile> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusCustomerDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    const qb = this.repository
      ...
      .select('customers.status', 'status')
      .addSelect('COUNT(*)', 'count')
      .where('customers.isDeleted = false')
      .groupBy('customers.status');
    if (organizationId) {
      qb.andWhere('customers.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<CustomerProfile[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<CustomerProfile[]> {
    const qb = this.repository.createQueryBuilder('customers');
    qb.where('customers.isDeleted = false');
    if (organizationId) {
      qb.andWhere('customers.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`customers.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QueryCustomerDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<CustomerProfile> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<CustomerProfile> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<CustomerProfile> {
    const entity = await this.findOne(id);
