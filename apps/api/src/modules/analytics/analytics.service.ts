...
export class AnalyticsService {
  ...
  constructor(
    ...
    private readonly repository: Repository<AnalyticsSnapshot>,
  ) {}

  async create(dto: CreateAnalyticsDto, userId: string): Promise<AnalyticsSnapshot> {
    this.logger.log(`Creating analytics by ${userId}`);
    ...
    this.logger.debug(`Created analytics ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateAnalyticsDto, userId: string): Promise<AnalyticsSnapshot[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QueryAnalyticsDto): Promise<PaginatedAnalyticsResponseDto> {
    const page = query.page || 1;
    const limit = Math.min(query.limit || 20, 200);
    const where: FindOptionsWhere<AnalyticsSnapshot> = {};
    ...
    if (query.search) {
      qb.andWhere(
        '(analytics.notes ILIKE :search OR analytics.externalRef ILIKE :search OR analytics.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('analytics.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`analytics.${sortBy}`, sortOrder);
  ...
  async findOne(id: string): Promise<AnalyticsSnapshot> {
  ...
  async update(id: string, dto: UpdateAnalyticsDto, userId: string): Promise<AnalyticsSnapshot> {
  ...
  async restore(id: string, userId: string): Promise<AnalyticsSnapshot> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusAnalyticsDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    const qb = this.repository
      ...
      .where('analytics.isDeleted = false')
      .groupBy('analytics.status');
    if (organizationId) {
      qb.andWhere('analytics.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<AnalyticsSnapshot[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<AnalyticsSnapshot[]> {
    const qb = this.repository.createQueryBuilder('analytics');
    qb.where('analytics.isDeleted = false');
    if (organizationId) {
      qb.andWhere('analytics.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`analytics.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QueryAnalyticsDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<AnalyticsSnapshot> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<AnalyticsSnapshot> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<AnalyticsSnapshot> {
    const entity = await this.findOne(id);
