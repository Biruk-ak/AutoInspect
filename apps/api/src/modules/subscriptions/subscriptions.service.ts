...
import {
  ...
  BulkUpdateStatusSubscriptionDto,
...
export class SubscriptionService {
  ...
  async create(dto: CreateSubscriptionDto, userId: string): Promise<SubscriptionPlan> {
    this.logger.log(`Creating subscriptions by ${userId}`);
    ...
    this.logger.debug(`Created subscriptions ${saved.id}`);
    return saved;
  }

  async bulkCreate(dto: BulkCreateSubscriptionDto, userId: string): Promise<SubscriptionPlan[]> {
    if (!dto.items?.length) {
  ...
  async findAll(query: QuerySubscriptionDto): Promise<PaginatedSubscriptionResponseDto> {
    ...
    const qb = this.repository.createQueryBuilder('subscriptions');
    qb.where(where);

    if (query.search) {
      qb.andWhere(
        '(subscriptions.notes ILIKE :search OR subscriptions.externalRef ILIKE :search OR subscriptions.status ILIKE :search)',
        { search: `%${query.search}%` },
      );
    }

    if (query.from && query.to) {
      qb.andWhere('subscriptions.createdAt BETWEEN :from AND :to', {
        from: query.from,
        to: query.to,
      });
    }
    ...
    qb.orderBy(`subscriptions.${sortBy}`, sortOrder);
  ...
  async update(id: string, dto: UpdateSubscriptionDto, userId: string): Promise<SubscriptionPlan> {
  ...
  async restore(id: string, userId: string): Promise<SubscriptionPlan> {
  ...
  async bulkUpdateStatus(dto: BulkUpdateStatusSubscriptionDto, userId: string): Promise<number> {
    if (!dto.ids?.length) {
  ...
  async countByStatus(organizationId?: string): Promise<Record<string, number>> {
    const qb = this.repository
      ...
      .select('subscriptions.status', 'status')
      .addSelect('COUNT(*)', 'count')
      .where('subscriptions.isDeleted = false')
      .groupBy('subscriptions.status');
    if (organizationId) {
      qb.andWhere('subscriptions.organizationId = :organizationId', { organizationId });
    }
  ...
  async getRecent(limit = 10, organizationId?: string): Promise<SubscriptionPlan[]> {
  ...
  async searchByTags(tags: string[], organizationId?: string): Promise<SubscriptionPlan[]> {
    const qb = this.repository.createQueryBuilder('subscriptions');
    qb.where('subscriptions.isDeleted = false');
    if (organizationId) {
      qb.andWhere('subscriptions.organizationId = :organizationId', { organizationId });
    }
    for (let i = 0; i < tags.length; i++) {
      qb.andWhere(`subscriptions.tags LIKE :tag${i}`, { [`tag${i}`]: `%${tags[i]}%` });
    }
    return qb.getMany();
  }

  async exportCsv(query: QuerySubscriptionDto): Promise<string> {
  ...
  async clone(id: string, userId: string): Promise<SubscriptionPlan> {
  ...
  async attachMetadata(id: string, patch: Record<string, unknown>, userId: string): Promise<SubscriptionPlan> {
    const entity = await this.findOne(id);
  ...
  async setTags(id: string, tags: string[], userId: string): Promise<SubscriptionPlan> {
    const entity = await this.findOne(id);
