...
export class SettingsController {
  ...
  @ApiOperation({ summary: 'Create settings record' })
  create(@Body() dto: CreateSettingsDto, @CurrentUser() user: { id: string }) {
    return this.service.create(dto, user.id);
  }

  @Post('bulk')
  @Roles('admin', 'garage')
  @ApiOperation({ summary: 'Bulk create settings records' })
  bulkCreate(@Body() dto: BulkCreateSettingsDto, @CurrentUser() user: { id: string }) {
    return this.service.bulkCreate(dto, user.id);
  }

  @Get()
  @Roles('admin', 'inspector', 'garage', 'customer')
  @ApiOperation({ summary: 'List settings records' })
  ...
  @ApiOperation({ summary: 'Count settings by status' })
  ...
  @Get('recent')
  @Roles('admin', 'inspector', 'garage', 'customer')
  @ApiOperation({ summary: 'Recent settings records' })
  recent(
  ...
  @ApiOperation({ summary: 'Export settings as CSV' })
  exportCsv(@Query() query: QuerySettingsDto) {
  ...
  @Post('bulk/status')
  @Roles('admin')
  bulkStatus(@Body() dto: BulkUpdateStatusSettingsDto, @CurrentUser() user: { id: string }) {
