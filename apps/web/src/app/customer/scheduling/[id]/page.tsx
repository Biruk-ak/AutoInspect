...
export default function SchedulingDetailPage() {
  ...
  const { token } = useAuth();
  const { push } = useToast();
  const [record, setRecord] = useState<SchedulingRecord | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchSchedulingById(token, params.id)
      .then((data) => {
        setRecord(data);
        setNotes(data.notes || '');
      })
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      const updated = await updateScheduling(token, record.id, { notes });
      setRecord(updated);
