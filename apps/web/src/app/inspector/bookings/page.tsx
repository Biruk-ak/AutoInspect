...
export default function BookingPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<BookingRecord> = await fetchBookingList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      push({ type: 'success', message: 'Booking created' });
      await load();
    } catch (err) {
  ...
  const columns = [
    {
      ...
      render: (row: BookingRecord) => (
        <Link href={'/inspector/bookings/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    ...
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: BookingRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: BookingRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: BookingRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Booking"
        subtitle="inspector portal — manage bookings for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
