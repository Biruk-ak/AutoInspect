...
export default function ReportPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<ReportRecord> = await fetchReportList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      push({ type: 'success', message: 'Report created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateReport(token, id, { status: next });
  ...
  const columns = [
    {
      ...
      render: (row: ReportRecord) => (
        <Link href={'/inspector/reports/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: ReportRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: ReportRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: ReportRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: ReportRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Report"
        subtitle="inspector portal — manage reports for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
