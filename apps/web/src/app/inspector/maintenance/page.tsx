...
export default function MaintenancePage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<MaintenanceRecord> = await fetchMaintenanceList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      push({ type: 'success', message: 'Maintenance created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateMaintenance(token, id, { status: next });
  ...
  const columns = [
    {
      ...
      render: (row: MaintenanceRecord) => (
        <Link href={'/inspector/maintenance/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    ...
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: MaintenanceRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: MaintenanceRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: MaintenanceRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Maintenance"
        subtitle="inspector portal — manage maintenance for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
          ...
          >
            {creating ? 'Creating…' : 'New Maintenance'}
          </button>
        }
      />
