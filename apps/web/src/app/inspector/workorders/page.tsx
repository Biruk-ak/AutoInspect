...
export default function WorkOrderPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<WorkOrderRecord> = await fetchWorkOrderList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      push({ type: 'success', message: 'WorkOrder created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateWorkOrder(token, id, { status: next });
  ...
  const onDelete = async (id: string) => {
    if (!token) return;
    try {
      await deleteWorkOrder(token, id);
  ...
  const columns = [
    {
      key: 'id',
      label: 'ID',
      render: (row: WorkOrderRecord) => (
        <Link href={'/inspector/workorders/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: WorkOrderRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: WorkOrderRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: WorkOrderRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: WorkOrderRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="WorkOrder"
        subtitle="inspector portal — manage workorders for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
          ...
          >
            {creating ? 'Creating…' : 'New WorkOrder'}
          </button>
        }
      />
      ...
      {loading ? <LoadingSpinner label="Loading workorders…" /> : null}
