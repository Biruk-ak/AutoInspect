...
export default function SettingsPage() {
  ...
  const { push } = useToast();
  const [items, setItems] = useState<SettingsRecord[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<SettingsRecord> = await fetchSettingsList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    setCreating(true);
    try {
      await createSettings(token, {
        status: 'draft',
        notes: 'Created from inspector portal by ' + (user?.email || 'unknown'),
        organizationId: user?.organizationId,
        priority: 50,
      });
      push({ type: 'success', message: 'Settings created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateSettings(token, id, { status: next });
  ...
  const columns = [
    {
      key: 'id',
      label: 'ID',
      render: (row: SettingsRecord) => (
        <Link href={'/inspector/settings/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: SettingsRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: SettingsRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: SettingsRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: SettingsRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Settings"
        subtitle="inspector portal — manage settings for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
          ...
          >
            {creating ? 'Creating…' : 'New Settings'}
          </button>
        }
      />
