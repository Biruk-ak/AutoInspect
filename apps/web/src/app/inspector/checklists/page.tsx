...
export default function ChecklistPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<ChecklistRecord> = await fetchChecklistList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    setCreating(true);
    try {
      await createChecklist(token, {
        status: 'draft',
        notes: 'Created from inspector portal by ' + (user?.email || 'unknown'),
        organizationId: user?.organizationId,
        priority: 50,
      });
      push({ type: 'success', message: 'Checklist created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateChecklist(token, id, { status: next });
  ...
  const columns = [
    {
      key: 'id',
      label: 'ID',
      render: (row: ChecklistRecord) => (
        <Link href={'/inspector/checklists/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: ChecklistRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: ChecklistRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: ChecklistRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: ChecklistRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Checklist"
        subtitle="inspector portal — manage checklists for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
      ...
      {loading ? <LoadingSpinner label="Loading checklists…" /> : null}
