...
export default function RegionPage() {
  ...
  const { push } = useToast();
  const [items, setItems] = useState<RegionRecord[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<RegionRecord> = await fetchRegionList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      push({ type: 'success', message: 'Region created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateRegion(token, id, { status: next });
  ...
  const columns = [
    {
      key: 'id',
      label: 'ID',
      render: (row: RegionRecord) => (
        <Link href={'/inspector/regions/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: RegionRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: RegionRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: RegionRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: RegionRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Region"
        subtitle="inspector portal — manage regions for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
      ...
      {loading ? <LoadingSpinner label="Loading regions…" /> : null}
