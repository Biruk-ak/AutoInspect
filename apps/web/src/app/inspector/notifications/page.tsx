...
export default function NotificationPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<NotificationRecord> = await fetchNotificationList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      push({ type: 'success', message: 'Notification created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateNotification(token, id, { status: next });
  ...
  const columns = [
    {
      ...
      render: (row: NotificationRecord) => (
        <Link href={'/inspector/notifications/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: NotificationRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: NotificationRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: NotificationRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: NotificationRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Notification"
        subtitle="inspector portal — manage notifications for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
          ...
          >
            {creating ? 'Creating…' : 'New Notification'}
          </button>
        }
      />
