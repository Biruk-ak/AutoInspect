...
export default function InsurancePage() {
  ...
  const { push } = useToast();
  const [items, setItems] = useState<InsuranceRecord[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<InsuranceRecord> = await fetchInsuranceList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    setCreating(true);
    try {
      await createInsurance(token, {
        status: 'draft',
        notes: 'Created from inspector portal by ' + (user?.email || 'unknown'),
        organizationId: user?.organizationId,
        priority: 50,
      });
      push({ type: 'success', message: 'Insurance created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateInsurance(token, id, { status: next });
  ...
  const onDelete = async (id: string) => {
    if (!token) return;
    try {
      await deleteInsurance(token, id);
  ...
  const columns = [
    {
      key: 'id',
      label: 'ID',
      render: (row: InsuranceRecord) => (
        <Link href={'/inspector/insurance/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: InsuranceRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: InsuranceRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: InsuranceRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: InsuranceRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Insurance"
        subtitle="inspector portal — manage insurance for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
          ...
          >
            {creating ? 'Creating…' : 'New Insurance'}
          </button>
        }
      />
      ...
      {loading ? <LoadingSpinner label="Loading insurance…" /> : null}
