...
export default function CompliancePage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<ComplianceRecord> = await fetchComplianceList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    setCreating(true);
    try {
      await createCompliance(token, {
        status: 'draft',
        notes: 'Created from inspector portal by ' + (user?.email || 'unknown'),
        organizationId: user?.organizationId,
        priority: 50,
      });
      push({ type: 'success', message: 'Compliance created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateCompliance(token, id, { status: next });
  ...
  const onDelete = async (id: string) => {
    if (!token) return;
    try {
      await deleteCompliance(token, id);
  ...
  const columns = [
    {
      ...
      render: (row: ComplianceRecord) => (
        <Link href={'/inspector/compliance/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: ComplianceRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: ComplianceRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: ComplianceRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: ComplianceRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Compliance"
        subtitle="inspector portal — manage compliance for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
          ...
          >
            {creating ? 'Creating…' : 'New Compliance'}
          </button>
        }
      />
      ...
      {loading ? <LoadingSpinner label="Loading compliance…" /> : null}
