...
export default function DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: ListResponse<DocumentRecord> = await fetchDocumentList(token, query);
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      push({ type: 'success', message: 'Document created' });
      await load();
    } catch (err) {
  ...
  const onUpdateStatus = async (id: string, next: string) => {
    if (!token) return;
    try {
      await updateDocument(token, id, { status: next });
  ...
  const columns = [
    {
      ...
      render: (row: DocumentRecord) => (
        <Link href={'/inspector/documents/' + row.id} className="text-sky-700 underline">
          {row.id.slice(0, 8)}
        </Link>
      ),
    },
    {
      key: 'status',
      label: 'Status',
      render: (row: DocumentRecord) => <StatusBadge status={row.status} />,
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (row: DocumentRecord) => formatCurrency(row.amount || 0, row.currency || 'USD'),
    },
    {
      key: 'priority',
      label: 'Priority',
      render: (row: DocumentRecord) => <span>{row.priority}</span>,
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (row: DocumentRecord) => formatDate(row.createdAt),
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <PageHeader
        title="Document"
        subtitle="inspector portal — manage documents for AutoInspect"
        actions={
          <button
            type="button"
            disabled={creating}
