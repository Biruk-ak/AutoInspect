...
export function isTerminalChecklistStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionChecklist(from: string, to: string): boolean {
...
export function normalizeChecklistTags(tags: string[]): string[] {
