...
export function isTerminalCertificateStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionCertificate(from: string, to: string): boolean {
...
export function normalizeCertificateTags(tags: string[]): string[] {
