...
export function isTerminalVehicleStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionVehicle(from: string, to: string): boolean {
...
export function normalizeVehicleTags(tags: string[]): string[] {
