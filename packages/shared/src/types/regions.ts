...
export function isTerminalRegionStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionRegion(from: string, to: string): boolean {
...
export function normalizeRegionTags(tags: string[]): string[] {
