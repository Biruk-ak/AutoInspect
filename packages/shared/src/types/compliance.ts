...
export function isTerminalComplianceStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionCompliance(from: string, to: string): boolean {
...
export function normalizeComplianceTags(tags: string[]): string[] {
