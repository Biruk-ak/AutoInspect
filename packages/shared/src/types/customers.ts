...
export function isTerminalCustomerStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionCustomer(from: string, to: string): boolean {
...
export function normalizeCustomerTags(tags: string[]): string[] {
