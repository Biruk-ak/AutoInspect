...
export function isTerminalInsuranceStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionInsurance(from: string, to: string): boolean {
...
export function normalizeInsuranceTags(tags: string[]): string[] {
