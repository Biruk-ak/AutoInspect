...
export function isTerminalInspectorStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionInspector(from: string, to: string): boolean {
...
export function normalizeInspectorTags(tags: string[]): string[] {
