...
export function isTerminalSchedulingStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionScheduling(from: string, to: string): boolean {
...
export function normalizeSchedulingTags(tags: string[]): string[] {
