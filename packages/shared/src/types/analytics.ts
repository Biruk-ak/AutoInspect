...
export function isTerminalAnalyticsStatus(status: string): boolean {
  return ['completed', 'cancelled', 'archived', 'failed'].includes(status);
}

export function canTransitionAnalytics(from: string, to: string): boolean {
...
export function normalizeAnalyticsTags(tags: string[]): string[] {
